...
func ResponsesStreamEventToChatChunks(event *ResponsesStreamResponse, state *ResponsesToChatStreamState) ([]ChatCompletionsStreamResponse, error) {
...
func FinalizeResponsesToChatStream(state *ResponsesToChatStreamState) []ChatCompletionsStreamResponse {
	if state == nil {
		return nil
	}
...
func (s *ResponsesToChatStreamState) applyResponseMetadata(response *OpenAIResponsesResponse) {
	if response == nil {
		return
	}
	if response.ID != "" && s.ID == "" {
...
func (s *ResponsesToChatStreamState) ensureStart() []ChatCompletionsStreamResponse {
	if s.sentStart {
		return nil
	}
	s.sentStart = true
	return []ChatCompletionsStreamResponse{s.makeChunk(ChatCompletionsStreamResponseChoiceDelta{
		Role:    "assistant",
		Content: getPointer(""),
	}, nil)}
}

func (s *ResponsesToChatStreamState) textDelta(delta string) []ChatCompletionsStreamResponse {
	if delta == "" {
		return nil
	}
	...
	chunks = append(chunks, s.makeChunk(ChatCompletionsStreamResponseChoiceDelta{
		Content: &delta,
	}, nil))
	return chunks
}

func (s *ResponsesToChatStreamState) terminalOutputChunks(response *OpenAIResponsesResponse) []ChatCompletionsStreamResponse {
	...
	for i := range response.Output {
		...
		case isResponsesToolOutputType(out.Type):
			chunks = append(chunks, s.toolItem(&ResponsesStreamResponse{Item: out})...)
		}
	}
	return chunks
}

func (s *ResponsesToChatStreamState) reasoningDelta(delta string) []ChatCompletionsStreamResponse {
	if delta == "" {
		return nil
	}
...
func (s *ResponsesToChatStreamState) toolItem(event *ResponsesStreamResponse) []ChatCompletionsStreamResponse {
...
func (s *ResponsesToChatStreamState) toolArgumentsDelta(event *ResponsesStreamResponse) []ChatCompletionsStreamResponse {
	if event.Delta == "" {
		return nil
	}
...
func (s *ResponsesToChatStreamState) flushPendingTool(event *ResponsesStreamResponse) []ChatCompletionsStreamResponse {
	tool := s.findToolForEvent(event)
	if tool == nil {
...
func (s *ResponsesToChatStreamState) ensureToolForEvent(event *ResponsesStreamResponse) *responsesStreamTool {
...
func (s *ResponsesToChatStreamState) findToolForEvent(event *ResponsesStreamResponse) *responsesStreamTool {
	if event == nil {
		return nil
	}
	if event.OutputIndex != nil {
...
func (s *ResponsesToChatStreamState) ensureFallbackToolForEvent(event *ResponsesStreamResponse) *responsesStreamTool {
	if event == nil {
		return nil
	}
	key := ""
...
func (s *ResponsesToChatStreamState) toolDelta(tool *responsesStreamTool, explicitDelta string) []ChatCompletionsStreamResponse {
	if tool == nil {
		return nil
	}
	...
	chunks = append(chunks, s.makeChunk(ChatCompletionsStreamResponseChoiceDelta{
		ToolCalls: []ToolCallResponse{responseTool},
	}, nil))
	s.sawToolCall = true
	return chunks
}

func (s *ResponsesToChatStreamState) finalize(response *OpenAIResponsesResponse) []ChatCompletionsStreamResponse {
	if s.finalized {
		return nil
	}
	s.finalized = true
	...
	chunks = append(chunks, s.makeChunk(ChatCompletionsStreamResponseChoiceDelta{}, &finishReason))
	if s.IncludeUsage && s.Usage != nil {
		chunks = append(chunks, ChatCompletionsStreamResponse{
			Id:      s.ID,
			Object:  "chat.completion.chunk",
			Created: s.Created,
			Model:   s.Model,
			Choices: make([]ChatCompletionsStreamResponseChoice, 0),
			Usage:   s.Usage,
		})
	}
	return chunks
...
func (s *ResponsesToChatStreamState) flushAllPendingTools() []ChatCompletionsStreamResponse {
...
func (s *ResponsesToChatStreamState) makeChunk(delta ChatCompletionsStreamResponseChoiceDelta, finishReason *string) ChatCompletionsStreamResponse {
...
func (s *ResponsesToChatStreamState) keyForEvent(event *ResponsesStreamResponse) string {
	if event == nil {
		return ""
	}
	if event.OutputIndex != nil {
...
func (a *ResponsesBufferedAccumulator) ProcessEvent(event *ResponsesStreamResponse) {
	if a == nil || event == nil {
		return
	}
	switch event.Type {
...
func (a *ResponsesBufferedAccumulator) SupplementResponseOutput(resp *OpenAIResponsesResponse) {
...
func (a *ResponsesBufferedAccumulator) BuildOutput() []ResponsesOutput {
	if a == nil {
		return nil
	}
	out := make([]ResponsesOutput, 0, 2+len(a.tools))
	...
	for _, tool := range a.tools {
		...
		argsRaw, _ := marshalJSON(tool.Arguments.String())
		out = append(out, ResponsesOutput{
...
func (a *ResponsesBufferedAccumulator) ensureTool(event *ResponsesStreamResponse) *responsesBufferedTool {
	if idx, ok := a.findToolIndex(event); ok {
		tool := a.tools[idx]
...
func (a *ResponsesBufferedAccumulator) applyToolMetadata(tool *responsesBufferedTool, event *ResponsesStreamResponse) {
...
func (a *ResponsesBufferedAccumulator) findToolIndex(event *ResponsesStreamResponse) (int, bool) {
	if event == nil {
		return 0, false
	}
	if event.OutputIndex != nil {
